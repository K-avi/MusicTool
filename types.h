...
typedef struct { 
  ...
  bool (*compare) (S_SCALE scl1, S_SCALE scl2);

  S_SCALE (*rand)( LENGTH l);
  S_SCALE (*parse)(char *str );
  void (*print)(S_SCALE scale );
  ...
  S_INTERVAL_VECTOR (*scl_to_intvect)(S_SCALE scale, LENGTH length);
  ...
  void (*print_saved) (S_USERINFO * uinfo, INDEX index);
  void (*print_env)( S_USERINFO*uinfo);

}SCL_FUNC; 

typedef struct { 

  bool (*compare) (S_MODES m1, S_MODES m2);

  S_MODES (*rand)( LENGTH l);
  S_MODES (*parse)(char *str );
  void (*print)(S_MODES);
  
  S_MODES (*get)(S_USERINFO *uinfo, INDEX index);
  S_MODES (*fromscale) (S_SCALE scl);
...
typedef struct { 
  ...
  bool (*compare) (S_CHORD_PROG* chp1, S_CHORD_PROG* chp2);

  S_CHORD_PROG* (*rand)( S_SCALE a1, u_char a2);
  S_CHORD_PROG* (*parse)(char *str );
  void (*print)(S_CHORD_PROG * chprog);
  
  S_CHORD_PROG* (*get)(S_USERINFO *uinfo, INDEX index);
  S_SCALE (*toscale) (const S_CHORD_PROG* chprog);
...
typedef struct { 
  ...
  bool (*compare) (S_DODEC s1, S_DODEC s2);

  S_DODEC (*rand)();
  S_DODEC (*shuflle) (S_DODEC seed, u_long num);
  S_DODEC (*parse)(char *str );
  void (*print) (S_DODEC);

  S_DODEC (*get)(S_USERINFO *uinfo, INDEX index);

  S_DODEC * (*gen_matrix)(S_DODEC dodec);
  S_DODEC (*retrograde) (S_DODEC dodec);
  S_DODEC (*inverse) (S_DODEC dodec);
  S_DODEC (*ret_inv)(S_DODEC);

  S_DODEC (*nth_prime)(S_DODEC dodec, INDEX n); 
  S_DODEC (*nth_inv)(S_DODEC dodec, INDEX n);
  S_DODEC (*nth_ret)(S_DODEC dodec, INDEX n);
  S_DODEC (*nth_ret_in)(S_DODEC dodec, INDEX n);
...
typedef struct{
  DODEC_FUNC* dodec_funcs;
  CHPROG_FUNC* chprog_funcs;
  HARMO_FUNC* harmo_funcs;
  SCL_FUNC* scl_funcs;
}ALL_FUNC;
